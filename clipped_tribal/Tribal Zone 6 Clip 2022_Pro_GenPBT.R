...
## Steelhead harvest stock composition for Tribal Zone 6 clipped catch 2022. Run 5/8/23 for proportions.
...
setwd("C:/Users/abyrne/Documents/TAC/Steelhead PBT-GSI sampling/2022 Fishery/Tribal Zone 6/Output proportion clip/")
...
fishData <- read.csv("Tribal Zone 6 Clip 2022 data.csv", stringsAsFactors = F)  # sample biodata
...
#clipharvest <- read.csv("Tribal Zone 6 20xx Clip harvest.csv", stringsAsFactors = F) # use when there is clip and unclip harvest (samples)
  #unclipharvest <- read.csv("Tribal Zone 6 catch.csv", stringsAsFactors = F)  # use when there is clip and unclip harvest (samples)
 
allharvest<- read.csv("Tribal Zone 6 Clip 2022 catch for pro.csv", stringsAsFactors = F)  ## use when there is only clipped harvest (samples)
...
Run = "Tribal Zone 6 2022 Pro_H", RTYPE = "clipped", Hierarch_variables = c("pbt_Rgroup","Stock"), 
SizeCut = NULL, alph = a_1, B = 10000, writeBoot = T, pbtRates = tagrate_info, 
adClipVariable = "Adipose", physTagsVariable = "PhysTag", pbtGroupVariable = "pbt_Rgroup", dataGroupVariable = "week", screenOutput = "Tribal Zone 6 2022 Pro_H.txt",spibetr=TRUE)

## Set output file names for summaries--add other name for HNC and Wild if unclipped samples present)
    Run = "Tribal Zone 6 2022 Pro_H.csv"  # for naming csv H clipped file with totals and CIs from AB sum Loops
   
 
##### Loop 1 - get total and CI by Assign type (all PBT and all GSI)

Results <- read.delim("Tribal Zone 6 2022 Pro_H_Estim_Grand_Totals_Hier_Stock.txt", header=TRUE)
...
Boots <- read.delim("Tribal Zone 6 2022 Pro_H_Boot_Hier_Stock.txt", header=TRUE)  
...
##### Loop 3 -- totals by Stock and BY for PBT assigned pbt_Rgroups 
  ...
  stocktotal <- read.delim("Tribal Zone 6 2022 Pro_H_Estim_Grand_Totals_Hier_pbt_Rgroup.txt", header=TRUE) ## use totals
   stock.boots <- read.delim("Tribal Zone 6 2022 Pro_H_Boot_Hier_pbt_Rgroup.txt", header=TRUE) ## iterations for CI
...
# get CIs for Stock and BY
 ...
 ##### Loop 3.a  -- get totals and Cis by Stock (add all BYs)
  ...
  marginalize_SD(estimatesFile="Tribal Zone 6 2022 Pro_H_Estim_Grand_Totals_Hier_Stock.txt",bootHier="Tribal Zone 6 2022 Pro_H_Boot_Hier_Stock.txt",marginalize = c("pbt_Rgroup"),alph=a_1)
 
##### Loop 4  -- Totals and CIs by Basin for all samples 
  
stockbasin<- read.delim("Tribal Zone 6 2022 Pro_H_Estim_Grand_Totals_Hier_Stock.txt", header=TRUE) ## use totals
 stockbasinci<- read.delim("Tribal Zone 6 2022 Pro_H_Boot_Hier_Stock.txt", header=TRUE) ## for Cis
